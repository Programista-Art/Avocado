...
begin
  ...
  if StartsText('jesli ', TrimmedLine) then
  begin
    // Dzielimy na warunek i resztę z uwzględnieniem spacji
    InstrukcjaWarunkowa := TrimmedLine.Split([' wtedy '], 2);

    if Length(InstrukcjaWarunkowa) = 2 then
    begin
      // Dzielimy część po THEN na ELSE i resztę
      Parts := InstrukcjaWarunkowa[1].Split([' inaczej '], 2);

      // Przetwarzanie bloku THEN
      KodWtedy := PrzetworzBlok(Parts[0]);

      // Przetwarzanie bloku ELSE
      KodInaczej := '';
      if Length(Parts) > 1 then
        KodInaczej := PrzetworzBlok(Parts[1]);

      PascalCode.Add(JesliWtedyInaczej(InstrukcjaWarunkowa[0], KodWtedy, KodInaczej));
      Exit;
    end;
  end
